...
[task_local]
#京东评价
52 10 * * * https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_evaluation.js, tag=京东评价, enabled=true

================Loon==============
[Script]
cron "52 10 * * *" script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_evaluation.js,tag=京东评价

===============Surge=================
京东评价 = type=cron,cronexp="52 10 * * *",wake-system=1,timeout=3600,script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_evaluation.js

============小火箭=========
京东评价 = type=cron,script-path=https://raw.githubusercontent.com/jiulan/platypus/main/scripts/jd_evaluation.js, cronexpr="52 10 * * *", timeout=3600, enable=true
 */
const $ = new Env('京东评价');
...
function sendServiceEval(item){
    let url = `https://comment-api.jd.com/comment/sendDSR?pin=&_=${new Date()}&sceneval=2&g_login_type=1&callback=json&g_ty=ls`;
    let data = {
        'userclient': '29',
        'orderId': item["oid"],
        'otype': 1,
        ...
        $.get(options, async (err, resp, data) => {
            try {
                if (err) {
                    ...
                    console.log("JSON.parsedata",data);
                    data = JSON.parse(data);
                    if (data.errMsg === 'success') {
                        console.log('服务评价成功！');
                    }else {
...
function  generation(pname,usePname,type){
    ...
    let context = data[type]["开始"]+data[type]["中间"]+data[type]["结束"]+new Date();
    return context.replace('$',name)
}
function taskUrl(orderType,startPage,pageSize) {
    return {
        url: `https://wq.jd.com/bases/orderlist/list?order_type=${orderType}&start_page=${startPage}&last_page=0&page_size=${pageSize}&callersource=mainorder&traceid=&t=${new Date()}&sceneval=2&g_ty=ls&g_tk=5381`,
        headers: {
            'Accept': 'application/json',
            "Content-Type": "application/x-www-form-urlencoded",
