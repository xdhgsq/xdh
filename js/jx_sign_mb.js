  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookiesArr[i]) {
      ...
      if (isLoginInfo[$.UserName] === false) {
        console.log(`没有登陆信息的用户是 ${$.UserName}`)
      } else {
      ...
      pin = Md5.hashStr($.UserName)
       http.get(`http://cfd.212618.xyz/jxsign.php?type=save&pin=${pin}&sharecode=${code}`, function (res) {
        ...
        res.on('data', function (chunk) {
          ...
          console.log(jsonDecodeData);return false;
...
async function getShareCode () {
    ...
    let HELP_HW = 'true';
    if (HELP_HW === 'true') {
      let resultsX = await getAuthorShareCode("https://raw.githubusercontent.com/lllrrr3/cfd/main/jxsign.json");
      if(resultsX == '') {
