...
app.post('/checkCode', async function (request, response) {
  try {
    ...
    if (data.err_code > 0) {
      ...
      await child_process.exec(`echo "${cookie}" > /tmp/webaddcookie.txt && sh /usr/share/jd_openwrt_script/JD_Script/jd.sh addcookie concurrent_js_update`, function(err, sto) {
